...
在线计算器：[https://johnguo0915.github.io/ai-compute-advisor/](https://johnguo0915.github.io/ai-compute-advisor/)
...
当前内置版本为 `2026.09-sample.12`，属于方向性示例数据，并非实时市场行情。九类目录需要成套发布。建议原子替换整份 `dist/`；如果使用 CDN 缓存，要确保 manifest 和各目录文件来自同一次构建，避免新 manifest 与旧缓存数据混用。
...
内置目录当前提供 26 个有代表性的本地/开放权重模型，覆盖紧凑工作站、24–32 GB、48–72 GB 和大统一内存等规划区间；其中 Qwen3.6 35B、Gemma 4 26B 与 Ornith 1.5 35B 各有一条 AI Fusion 2.0 支持选项，KV Cache 按 K8-bit / Q3-bit 压缩（为同架构本地 Q8 字节数的 11/16），并规划约 5GB 模型权重卸载到系统内存、约 1GB KV Cache 放到 SSD、安全余量 5% 后重算 GPU 显存。它是可扩充的起始集合，不代表穷举市场全部模型。Artificial Analysis 的开放模型目录只用于发现主流模型家族，参数、上下文、许可与部署说明以模型发布方的模型卡为准。它提供的云端价格、速度、延迟或智能评分不会被静默当成本地 TPS 或硬件适配证据。用户仍可通过 Full Data Pack 或 Browser Library 新增、修改和删除模型。
